...
```
```{r}
glimpse(life_history) #use glimpse or summary to see what forms NA may take in the dataset
```
...
Multiple approaches were needed to deal with NA's in this data set because it contains NA's in the form of blank spaces in the table, "not measured", and as "-999". Different functions are needed to identify each type of NA.
...
```{r}
endangered <- readr::read_csv("data/cites.csv")
endangered <- janitor::clean_names(endangered)
```
```{r}
glimpse(endangered)
```
...
endangered %>%
  ...
  select(exporter_reported_quantity, taxon) %>%
  naniar::miss_var_summary(order=T)
```
