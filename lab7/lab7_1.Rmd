...
4. Given that we know that the output of #2 is wrong, what do you think is the best approach for dealing with NA's in the `amniote` data? Use the mutate_all() function. NAs may be written as "-999.00", so change any "-999.00" to "NA".
...
life_history %>%
  summary()
```
Did not detect any NAs using summary().
```{r}
life_history %>%
  summarise_all(~(sum(is.na(.))))
```
Reconfirmation of no NAs detected.
```{r}
...
life_history_tidy
```
NAs were written as "-999" and detected.
