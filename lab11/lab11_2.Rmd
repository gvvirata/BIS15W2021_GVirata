...
```{r}
p+theme_linedraw()+
  theme(legend.position = "top", #can get rid of legend using 'none'
    axis.text.x = element_text(angle = 60, hjust=1))+
  labs(title = "Observations by Taxon in Homerange Data",
       x = NULL,
       y= "n",
       fill= "Trophic Guild") #title of legend
```
```{r}
p+theme_classic()
```
...
```
```{r}
pdf("homerange.pdf") #file type and name of file with type extension
p2 #name of the plot I want to export
dev.off() 
```
