...
Since we have incomplete data, the "mean" function is unable to produce the actual mean of the set. Therefore, "NA" is produced rather than an error code.
...
```
This is useful because we can now take the mean of the set of numbers using the data that is already available to us. In this case, there was only one NA in the set so we could have simply deleted it manually rather than using "na.rm=T". However, in larger data sets where there may be multiple NAs, deleting all may be tedious or even difficult to find ourselves in the first place. Therefore, "na.rm" is much more useful.
