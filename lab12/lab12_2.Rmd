...
lat <- c(34.67, 41.80)
long <- c(-124.1, -115.5)
bbox <- make_bbox(long, lat, f = 0.05) #f=way of slightly expanding the bounding box beyond the coordinates to ensure all values show
```
...
```{r}
map1 <- get_map(bbox, maptype = "terrain", source = "stamen") #stamen is a free and open source
```
...
```{r}
ggplot() + 
  geom_sf(data = us_comp, size = 0.125) + #size=size of lines that show the state boundaries
  theme_linedraw()+
